...
async def scheduler_loop():
    ...
    while auto_scheduler_running:
        try:
            # 计算下次运行时间
            scheduler_next_run = datetime.now() + timedelta(minutes=scheduler_interval_minutes)
            ...
            logger.info(f"✅ 定时任务完成，下次运行: {scheduler_next_run.strftime('%H:%M:%S')}")
...
async def start_scheduler(interval_minutes: int = Query(default=30, description="调度间隔(分钟)")):
    """启动自动调度"""
    global auto_scheduler_running, scheduler_task, scheduler_interval_minutes
